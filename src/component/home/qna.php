...
<script>
	document.addEventListener('DOMContentLoaded', function() {
		...
		// Header animation
		gsap.from('.home-qna__header > *', {
			scrollTrigger: {
				trigger: '.home-qna__header',
				start: 'top 85%',
		...
		// QNA Items animation
		var qnaItems = document.querySelectorAll('.home-qna__item');
		qnaItems.forEach(function(item, i) {
			var startPct = Math.floor(75 + Math.random() * 15); // 75% - 90%
			var randDelay = Math.random() * 0.1;
			gsap.from(item, {
				scrollTrigger: {
					trigger: item,
					start: 'top ' + startPct + '%',
